...
/* ══════════════════════════════════════════════════════════
   BOTTOM BANNER  — reused on Home, About, Policy
══════════════════════════════════════════════════════════ */
  ...
  return (
    <div style={{ position:"relative", overflow:"hidden", marginTop:"2rem" }}>
      ...
      <div style={{
        ...
        <p style={{ fontFamily:"Georgia,serif", fontStyle:"italic", color:"rgba(253,245,228,0.72)", fontSize:"14px", margin:"0 0 1.5rem" }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:"16px", justifyContent:"center" }}>
          {[
            ...
            <div key={i} style={{ color:"#FDF5E4", fontFamily:"sans-serif", fontSize:"12px", display:"flex", alignItems:"center", gap:"6px" }}>
              <i className={`ti ${r.icon}`} style={{ color:GOLD, fontSize:"14px" }} />
              {r.text}
            </div>
          ))}
        </div>
    ...
    <div>
      ...
      <div style={{ textAlign:"center", padding:"2.5rem 1rem 1rem" }}>
        <button
          ...
          style={{
            background:NAVY, color:GOLD, border:`2px solid ${GOLD}`,
            borderRadius:"99px", padding:"14px 44px",
...
function MenuPage({ sectionIdx, setSectionIdx, setConfigItem }) {
  ...
  return (
    <div>
        ...
        {sections.map((s, i) => (
          <button key={s.id}
            ...
            style={{
              ...
              fontSize:"12px", fontWeight: i === sectionIdx ? 700 : 400,
      ...
      <div onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} style={{ minHeight:"60vh" }}>

        {/* Section header — unified navy + gold */}
        <div style={{ background:NAVY, padding:"14px 20px", display:"flex", alignItems:"center", gap:"12px" }}>
          <div style={{
        ...
        <div style={{
          ...
          padding:"16px 20px", background:"#F9F3E8",
        }}>
          <button
  ...
  return (
    <div>
      ...
      <div style={{ background:NAVY2, padding:"3rem 1.5rem 2rem", textAlign:"center" }}>
        <img src="/logo.png" alt="Jiko La Bibi JJJ" width={120} height={120}
          style={{ borderRadius:"50%", border:`3px solid ${GOLD}`, objectFit:"cover", marginBottom:"1rem" }}
          onError={e => { e.target.style.display = "none"; }}
        />
        <h1 style={{ fontFamily:"Georgia,serif", fontSize:"26px", fontWeight:900, color:"#FDF5E4", margin:"0 0 6px" }}>
          Kuhusu Sisi
        </h1>
        <p style={{ fontFamily:"Georgia,serif", fontStyle:"italic", color:"rgba(253,245,228,0.70)", fontSize:"15px", margin:0 }}>
          Taste of Tanzania in DAR
        </p>
      </div>
      ...
      <div style={{ maxWidth:"720px", margin:"0 auto", padding:"2rem 1rem" }}>
        ...
        <div style={{
          ...
          marginBottom:"1.5rem", boxShadow:"0 3px 14px rgba(11,31,69,0.08)",
          borderLeft:`5px solid ${GOLD}`,
        }}>
          <h2 style={{ fontFamily:"Georgia,serif", fontSize:"20px", color:NAVY, margin:"0 0 1rem" }}>
            🏡 Hadithi Yetu
          </h2>
        ...
        <div style={{ display:"grid", gap:"12px", gridTemplateColumns:"repeat(auto-fill, minmax(260px, 1fr))" }}>
          {info.map((item, i) => (
            <div key={i} style={{
              ...
              boxShadow:"0 2px 10px rgba(11,31,69,0.07)",
              display:"flex", gap:"12px", alignItems:"flex-start",
            }}>
              <i className={`ti ${item.icon}`} style={{ fontSize:"20px", color:GOLD, flexShrink:0, marginTop:"2px" }} />
              <div>
                <div style={{ fontSize:"10px", fontFamily:"sans-serif", fontWeight:700, color:"rgba(11,31,69,0.4)", letterSpacing:"0.8px", textTransform:"uppercase", marginBottom:"3px" }}>
                  {item.label}
                </div>
                <div style={{ fontSize:"14px", fontFamily:"Georgia,serif", color:NAVY }}>
                  {item.value}
                </div>
              </div>
            </div>
...
function PolicyPage() {
  const policies = [
    {
      icon:"ti-clipboard-list", title:"1. Sera ya Maagizo",
      body:"• Maagizo yanakubaliwa kupitia WhatsApp, simu, au tovuti yetu\n• Chakula cha kawaida hutayarishwa ndani ya dakika 15–30\n• Maagizo ya matukio (Events) yanahitaji siku 2–3 za mapema\n• Tunashughulikia: Kuchukua, Delivery, Kula Hapa, na Matukio",
    },
    {
      icon:"ti-credit-card", title:"2. Sera ya Malipo",
      body:"• Tunakubali: Lipa Namba (18873261), Mixx by Yas, Airtel Money, na Pesa Taslimu\n• Malipo yanaweza fanywa kabla au wakati wa kupokea chakula\n• Risiti hutolewa kwa maombi yoyote",
    },
    {
      icon:"ti-bike", title:"3. Sera ya Delivery",
      body:"• Tunahudumia maeneo ya Dar es Salaam\n• Gharama ya delivery inategemea umbali — wasiliana nasi kwa maelezo\n• Muda wa delivery: ndani ya saa 1 baada ya chakula kutayarishwa",
    },
    {
      icon:"ti-refresh", title:"4. Sera ya Kurejesha Pesa (Refund)",
      body:"• Kurejesha pesa kunafanyika ikiwa agizo halikutimizwa kwa sababu yetu\n• Kanselo ya agizo baada ya dakika 15 (chakula kikiwa kinaandaliwa) linaweza kulipishwa asilimia 30\n• Malalamiko yashughulikiwe haraka — wasiliana kupitia WhatsApp mara moja",
    },
    {
      icon:"ti-shield-check", title:"5. Kanuni ya Chakula (Food Disclaimer)",
      body:"• Chakula chetu kinapikwa kwa viungo vya asili, bila kemikali za ziada\n• Hatuna dhamana kamili ya kukosa alerji — tafadhali tuambie mapema ukiwa na mzio wowote\n• Bei zinaweza kubadilika kulingana na hali ya soko bila taarifa ya awali\n• Tunafuata Sheria ya Chakula, Dawa na Vipodozi (Cap 219) ya Tanzania\n• Leseni ya Biashara: BL01396922025-2605162556 | TIN: 192-791-243\n• Mkoa: Dar es Salaam, Wilaya ya Ubungo (Kinondoni Tax Region)",
    },
    {
      icon:"ti-lock", title:"6. Sera ya Faragha (Privacy Policy)",
      body:"• Habari zako za kibinafsi (jina, nambari ya simu) zinakingwa na sisi\n• Tunatumia maelezo yako kwa mawasiliano ya agizo lako pekee\n• Hatutashiriki taarifa zako na taasisi au watu wa nje bila ruhusa yako wazi",
    },
  ];
  ...
  return (
    <div>
      <div style={{ background:NAVY2, padding:"2.5rem 1.5rem", textAlign:"center" }}>
        ...
        <p style={{ fontFamily:"Georgia,serif", fontStyle:"italic", color:"rgba(253,245,228,0.65)", fontSize:"14px", margin:0 }}>
          Unyamwezini Jiko La Bibi JJJ — Dar es Salaam
        </p>
      </div>
      ...
      <div style={{ maxWidth:"720px", margin:"0 auto", padding:"1.5rem 1rem" }}>
        {policies.map((p, i) => (
          <div key={i} style={{
            ...
            marginBottom:"1rem", boxShadow:"0 2px 10px rgba(11,31,69,0.07)",
            borderLeft:`5px solid ${GOLD}`,
          }}>
            <h3 style={{
              fontFamily:"Georgia,serif", fontSize:"16px", fontWeight:700, color:NAVY,
              margin:"0 0 0.8rem", display:"flex", alignItems:"center", gap:"8px",
            }}>
              <i className={`ti ${p.icon}`} style={{ color:GOLD, fontSize:"18px" }} />
              {p.title}
            </h3>
            <p style={{
              fontFamily:"sans-serif", fontSize:"13.5px",
              color:"rgba(11,31,69,0.72)", lineHeight:1.85,
        ...
        <div style={{
          background:"rgba(212,175,55,0.10)", border:`1px solid rgba(212,175,55,0.40)`,
          borderRadius:"12px", padding:"1.25rem", textAlign:"center",
        }}>
          <p style={{ fontFamily:"sans-serif", fontSize:"13px", color:"rgba(11,31,69,0.65)", margin:0 }}>
            Maswali yoyote kuhusu sera hizi — wasiliana nasi kupitia WhatsApp:<br />
            <strong style={{ color:NAVY }}>+255 655 709 024</strong>
          </p>
        </div>
      </div>
